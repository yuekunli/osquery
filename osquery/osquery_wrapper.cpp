...
__attribute__((visibility("default"))) std::vector<std::map<std::string, std::string>>
...
extern "C" __attribute__((visibility("default"))) void genDispatchC(void* data)
{
	using ReturnType = std::vector<std::map<std::string, std::string>>;

	osquery::QueryContext ctx{};
	ReturnType result = osquery::tables::genKernelInfo(ctx);
	ReturnType& dataRef = *(static_cast<ReturnType*>(data));

	for (auto const& row : result)
	{
		dataRef.emplace_back();
		for (auto const& oneCol : row)
		{
			(*(dataRef.rbegin()))[oneCol.first] = oneCol.second;
		}
	}
}


extern "C" __attribute__((visibility("default"))) void genDispatchNoInput()
{
	osquery::QueryContext ctx{};
	std::vector<std::map<std::string, std::string>> data = osquery::tables::genKernelInfo(ctx);
...
extern "C" __attribute__((visibility("default"))) void* genDispatch2()
{
	using ReturnType = std::vector<std::map<std::string, std::string>>;
	...
	for(auto const& row : result)
	{
		...
		for (auto const& col : row)
		{
			data->rbegin()->operator [](col.first) = col.second;
		}
		std::cout<<"2 data size: "<< data->size() << std::endl;
	}

	std::cout<<"3 data size: "<< data->size() << std::endl;
